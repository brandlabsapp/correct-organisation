...
export function EstimateForm({ estimateId }: { estimateId?: string }) {
	...
	const [initialLoading, setInitialLoading] = useState(!!estimateId);
	...
	const fetchEstimate = async () => {
		try {
			...
			if (response.ok) {
				...
				setPreviewNumber(estimate.estimateNumber);
	...
	const fetchPreviewNumber = async () => {
		try {
			const response = await fetch(
				`/api/finance/estimates/preview-number?company=${companyId}`
			);
			if (response.ok) {
	...
	const calculateLineTotal = (item: LineItem) => {
		...
		const discountPercent = parseNumber(item.discountPercent || '0');
	...
	const calculateDiscountTotal = () => {
		return lineItems.reduce((sum, item) => {
			...
			return sum + (amount * parseNumber(item.discountPercent || '0')) / 100;
		}, 0);
	};
	...
	const calculateTaxTotal = () => {
		return lineItems.reduce((sum, item) => {
			const amount = parseNumber(item.quantity || '0') * parseNumber(item.rate || '0');
			const discountAmount = (amount * parseNumber(item.discountPercent || '0')) / 100;
			const afterDiscount = amount - discountAmount;
			return sum + (afterDiscount * parseNumber(item.taxRate || '0')) / 100;
		}, 0);
	};

	const calculateTotal = () => {
		return calculateSubtotal() - calculateDiscountTotal() + calculateTaxTotal();
	};
	...
	const handleSubmit = async (saveAsDraft: boolean = false) => {
		try {
			...
			const url = isEditing
				? `/api/finance/estimates/${estimateId}?company=${companyId}`
				: `/api/finance/estimates?company=${companyId}`;
			...
			if (response.ok) {
				...
				showSuccessToast({
					title: 'Success',
					message: `Estimate ${estimate.estimateNumber} ${isEditing ? 'updated' : 'created'} successfully`,
				});
				router.push(`/finance/estimates/${estimate.id}?company=${companyId}`);
			} else {
				const error = await response.json();
				showErrorToast({
					title: 'Error',
					message: error.message || `Failed to ${isEditing ? 'update' : 'create'} estimate`,
				});
			}
		} catch (error) {
			...
			showErrorToast({
				title: 'Error',
				message: `Failed to ${isEditing ? 'update' : 'create'} estimate`,
			});
		} finally {
			setLoading(false);
		}
		...
		<div className='p-6 max-w-5xl mx-auto space-y-6'>
			...
			<div className='flex items-center gap-4'>
				...
				<div>
					...
					<p className='text-gray-500'>Estimate # {previewNumber || '...'}</p>
				</div>
			</div>
						...
						>
							...
							<div className='space-y-2'>
								...
								<Input
									...
									onBlur={() => {
										const normalized = sanitizeNumericInput(item.discountPercent || '0', {
											allowDecimal: true,
											maxDecimals: 2,
										});
						...
						<div className='w-full md:w-80 space-y-3'>
							...
							{calculateDiscountTotal() > 0 && (
								<div className='flex justify-between text-sm text-green-600'>
									<span>Discount:</span>
									<span className='font-medium tabular-nums'>
										-{formatCurrency(calculateDiscountTotal(), formData.currency)}
									</span>
								</div>
							)}
							<div className='flex justify-between text-sm'>
